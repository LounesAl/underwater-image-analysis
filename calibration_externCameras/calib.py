...
calibration_settings = parse_calibration_settings_file(r'.\calibration_cameraPhone\calibration_settings.yaml')
    ...
    while True:
        ...
        if start:
            ...
            if cooldown <= 0:
                ...
                savename = os.path.join(CURRENT_PATH, f'frames_{camera_name}', '_' + id_no + '_' + str(saved_count) + '.png')
...
def save_frames_two_cams(camera0_name, camera1_name):
    ...
    while True:
        ...
        if not ret0 or not ret1:
            logging.info('Cameras not returning video data. Exiting...')
            sys.exit()
...
def stereo_calibrate(mtx0, dist0, mtx1, dist1, frames_prefix_c0, frames_prefix_c1):
    ...
    for frame0, frame1 in zip(c0_images, c1_images):
        gray1 = cv2.cv2tColor(frame0, cv2.COLOR_BGR2GRAY)
        gray2 = cv2.cv2tColor(frame1, cv2.COLOR_BGR2GRAY)
